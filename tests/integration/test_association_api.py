...
async def test_get_association(postgres, cn_db_engine, cn, client):
    response = await client.get(Paths.associations, params={"iri": cn.association_top["@id"]})
...
async def test_get_association_404(postgres, cn_db_engine, client):
    response = await client.get(
        Paths.associations, params={"iri": "http://pyst-tests.ninja/association/missing"}
    )
...
async def test_create_association(postgres, cn_db_engine, cn, client):
    ...
    response = await client.post(Paths.associations, json=new)
    assert response.status_code == 200
    given = response.json()
    for key, value in new.items():
        assert given[key] == value

    given = (await client.get(Paths.associations, params={"iri": new["@id"]})).json()
    for key, value in new.items():
        assert given[key] == value
