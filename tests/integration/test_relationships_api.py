...
async def test_get_relationships(postgres, cn_db_engine, relationships, client):
    response = await client.get(Paths.relationships, params={"iri": relationships[3].source})
...
async def test_get_relationships_args(postgres, cn_db_engine, relationships, client):
    response = await client.get(
        Paths.relationships, params={"iri": relationships[3].source, "source": 0, "target": 1}
    )
    assert response.status_code == 200
    assert response.json() == [
        {
...
async def test_get_relationships_empty(postgres, cn_db_engine, client):
    response = await client.get(
        Paths.relationships, params={"iri": "http://data.europa.eu/xsp/cn2024/woof"}
    )
    assert response.status_code == 200
    assert response.json() == []
...
async def test_create_relationships(postgres, cn_db_engine, client):
    ...
    response = await client.post(Paths.relationships, json=given)
    assert response.status_code == 200
    assert response.json() == given, "API return value incorrect"

    response = await client.get(Paths.relationships, params={"iri": "http://example.com/foo"})
    assert response.status_code == 200
...
async def test_create_relationships_duplicate(postgres, cn_db_engine, client, relationships):
    response = await client.post(Paths.relationships, json=[relationships[3].to_json_ld()])
...
async def test_create_relationships_across_scheme(
    ...
    await client.post(Paths.concept_schemes, json=new_scheme)
    ...
    await client.post(Paths.concepts, json=new_concept)
    ...
    response = await client.post(Paths.relationships, json=[cross_cs.to_json_ld()])
...
async def test_create_relationships_reference_concept_scheme(
    ...
    response = await client.post(Paths.relationships, json=[rel.to_json_ld()])
