...
async def test_concept_get_all_order(postgres, cn, graph):
    concepts = await graph.concept_get_all(cn.scheme["@id"], False)
    assert concepts
...
@pytest.mark.postgres
async def test_concept_get_all_no_concept_scheme_order(postgres, cn, graph):
    concepts = await graph.concept_get_all(None, False)
    assert concepts
    assert isinstance(concepts[0], Concept)
    assert [concept.id_ for concept in concepts] == sorted(
        [
...
@pytest.mark.postgres
async def test_concept_get_all_top_concepts(postgres, cn, graph):
    concepts = await graph.concept_get_all(cn.scheme_2023["@id"], True)
...
@pytest.mark.postgres
async def test_concept_get_all_no_concept_scheme_top_concepts(postgres, cn, graph):
    concepts = await graph.concept_get_all(None, True)
    assert concepts
    assert isinstance(concepts[0], Concept)
    assert [concept.id_ for concept in concepts] == sorted(
        [
...
@pytest.mark.postgres
async def test_concept_get_all_empty(postgres, cn, graph):
    concepts = await graph.concept_get_all("http://missing.ninja/foo", False)
    assert concepts == []
