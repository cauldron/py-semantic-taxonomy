...
async def test_concept_scheme_get(cn, anonymous_client, monkeypatch):
    ...
    response = await anonymous_client.get(get_full_api_path("concept_scheme"), params={"iri": cn.scheme["@id"]})
...
async def test_concept_scheme_get_not_found(cn, anonymous_client, monkeypatch):
    ...
    response = await anonymous_client.get(get_full_api_path("concept_scheme"), params={"iri": "foo"})
...
async def test_concept_scheme_create_unauthorized(anonymous_client):
    response = await anonymous_client.post(get_full_api_path("concept_scheme"), json={})
...
async def test_concept_scheme_create_error_validation_errors(cn, client, monkeypatch):
    ...
    response = await client.post(get_full_api_path("concept_scheme"), json=obj)
...
async def test_concept_scheme_update_unauthorized(anonymous_client):
    response = await anonymous_client.put(get_full_api_path("concept_scheme"), json={})
    assert response.status_code == 400
...
async def test_concept_scheme_delete_unauthorized(anonymous_client):
    response = await anonymous_client.delete(get_full_api_path("concept_scheme"), params={"iri": ""})
    assert response.status_code == 400
